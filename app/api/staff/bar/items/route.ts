...
export async function GET(request: NextRequest) {
  const auth = await requireStaffApiSession(["barra", "admin"]);

  if ("response" in auth) {
    return auth.response;
  }

  const parsedQuery = emptyBarQuerySchema.safeParse(
  ...
  if (error) {
    return errorResponse(
      "INTERNAL",
      getErrorMessage(error, "No pudimos cargar la cola de barra."),
      500,
    );
  }

  const parsedItems = barItemsResponseSchema.safeParse({ items: data });

  if (!parsedItems.success) {
    return errorResponse(
      "INTERNAL",
      "No pudimos interpretar la cola de barra.",
      500,
    );
  }
...
export async function PATCH(request: NextRequest) {
  ...
  const parsedBody = updateBarItemBodySchema.safeParse(jsonBody.data);
  ...
  if (!existingItem || existingItem.area !== "barra") {
    return errorResponse(
      "NOT_FOUND",
      "No encontramos ese ítem de barra.",
      404,
    );
  }
  ...
  const parsedItem = updateBarItemResponseSchema.safeParse({ item: updatedItem });
